#!/usr/bin/env python3
"""
DeerFlow Agent for SIMP System - Fixed Version 2
=================================================
Bridge between SIMP broker and DeerFlow agent orchestration system.
Uses correct DeerFlow API endpoints and request formats.
"""
...
class DeerFlowAgent:
        ...
        try:
            ...
            if hasattr(e, 'response') and e.response is not None:
                try:
                    ...
                    logger.error(f"DeerFlow API error details: {error_detail}")
                except:
                    pass
            raise
    ...
    def spawn_subagent(self, task_description: str, skill_id: Optional[str] = None) -> DeerFlowSubagent:
        ...
        # Create a unique agent name (DeerFlow requires lowercase alphanumeric + hyphens)
        timestamp = int(time.time())
        agent_name = f"simp-{timestamp}-{hash(task_description) % 10000:04d}"
        
        # Create SOUL.md content based on task
        soul_content = f"""# SIMP Subagent - {agent_name}

## Purpose
This agent was created by the SIMP (Structured Intent Messaging Protocol) system to execute the following task:

{task_description}

## Instructions
1. Execute the task described above
2. Report progress and results back to the SIMP system
3. Follow standard safety and ethical guidelines

## Created By
- Source: SIMP DeerFlow Agent
...
"""
        ...
        data = {
            "name": agent_name,
            "description": f"SIMP subagent for: {task_description[:100]}...",
            "soul": soul_content,
            "model": None,  # Use default model
            "tool_groups": ["default"]  # Use default tool groups
        }
        
        try:
            # Use the correct DeerFlow API endpoint with correct format
            result = self._deerflow_request("POST", "/api/agents", data)
            
            subagent = DeerFlowSubagent(
                agent_id=result.get("agent_id", agent_name),
                task_id=result.get("task_id", f"task-{agent_name}"),
                status="created",
            ...
            # Fallback: create a local subagent record even if DeerFlow API fails
            subagent = DeerFlowSubagent(
                agent_id=agent_name,
            ...
            self.subagents[subagent.agent_id] = subagent
            logger.warning(f"Created simulated subagent due to API error: {agent_name}")
            return subagent
