...
## Sprint 31-32 Follow-On (2026-04-07)

### Planner Compatibility
- Updated `bin/start_gemma4_agent.py` to accept all broker receive-path variants: `/intents/handle`, `/intents/receive`, and `/intent`.
- Added a background broker heartbeat loop in the managed Gemma4 startup path so the live planner advertises freshness every 30 seconds instead of relying only on re-registration or passive health probes.

### Dashboard Freshness Visibility
- Updated `dashboard/static/app.js` so heartbeat-stale agents render as a degraded `stale` badge instead of silently looking healthy.
- Added a lightweight regression assertion in `tests/test_sprint20_dashboard.py` to keep stale-status handling present in the dashboard client.

### Sprint 34-35 Follow-On (2026-04-07)
- Updated the Planner / Executor Flows table in `dashboard/static/index.html`, `dashboard/static/app.js`, and `dashboard/static/style.css` to show a compact timeline view rather than only IDs and statuses.
- The flow UI now renders per-step delivery status, duration, retries, and failure reason when the broker provides them, making the new broker telemetry directly visible to operators.
- Added a small regression assertion in `tests/test_sprint30_broker_flows.py` to keep the timeline markup and column present.

### Sprint 36 Follow-On (2026-04-07)
- Updated the Agent Smoke table in `dashboard/static/index.html` and `dashboard/static/app.js` to reflect real delivery-smoke results instead of assuming a simple health probe shape.
- The smoke panel now shows delivery status, probe path, latency, and degradation signal from the broker delivery smoke endpoint.

### Sprint 37 Security Hardening Follow-On (2026-04-07)
- Locked down previously unauthenticated broker surfaces in `simp/server/http_server.py`, including agent detail lookup, intent status/error recording, logs, and memory read/context-pack routes.
- Updated `simp/server/control_auth.py` so control-token checks read the current environment value on each request instead of freezing the token at module import time.
- Fixed newline-delimited socket framing in `simp/server/agent_client.py` so partial reads and multiple queued messages are buffered and decoded safely instead of assuming one `recv()` equals one JSON message.
- Added focused regression coverage in `tests/security/test_http_server_security.py` for API-key enforcement, control-token reload behavior, and socket message framing.
