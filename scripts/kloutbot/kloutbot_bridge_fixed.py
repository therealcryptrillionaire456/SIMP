...
def _setup():
    ...
    _post("/mesh/subscribe", {"agent_id": AGENT_ID, "channel": IN_CHANNEL, "simp_versions": ["1.0"]})
...
def _poll_instructions() -> list[dict]:
    """GET /mesh/subscribe/<channel>?agent_id=<id> — pulls queued messages."""
    try:
        r = requests.get(
            f"{BROKER_URL}/mesh/poll",
            params={"agent_id": AGENT_ID, "channel": IN_CHANNEL, "max_messages": 10},
            timeout=10,
        )
        r.raise_for_status()
