...
logger = logging.getLogger("SIMP.TaskLedger")
...
@dataclass
class LedgerConfig:
    """Tuning knobs for the task ledger."""
...
class TaskLedger:
    ...
    def append(self, record: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.error("TaskLedger.append failed: %s", exc)
            ...
            with open(self._path, "r", encoding="utf-8") as fh:
                for line in fh:
                    ...
                    try:
                        ...
                        logger.warning("TaskLedger: skipping corrupt line")
                        continue
                    if rec.get("status") == "pending":
                        results.append(rec)
        except Exception as exc:
            logger.error("TaskLedger.load_pending failed: %s", exc)
        return results
    ...
    def load_all(self, limit: int = 500) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error("TaskLedger.load_all failed: %s", exc)
        # Return last N
    ...
    def rotate_if_needed(self) -> bool:
        ...
        try:
            ...
            logger.info("TaskLedger rotated: %s -> %s", self._path, rotated)
            return True
        except Exception as exc:
            logger.error("TaskLedger.rotate failed: %s", exc)
            return False
