# KEEPTHECHANGE.com - System Architecture
## Instagram's Crypto Twin

## Overview
This document outlines the complete system architecture for KEEPTHECHANGE.com, detailing the technical components, their interactions, and the infrastructure required to support an Instagram-inspired social shopping platform with crypto investment features.
...
#### CDN & Edge Computing
**Components**:
- **CloudFront/Cloudflare**: Global content delivery
- **Lambda@Edge/Cloudflare Workers**: Edge functions for personalization
...
class SocialGraphService:
    ...
    async def get_followers(self, user_id: str) -> List[str]:
        ...
        query = """
            ...
            WHERE user_id = $1
        """
        followers = await self.db.fetch(query, user_id)
        
        # Cache results
        await self.redis.setex(
            cache_key, 
            self.cache_ttl, 
            json.dumps([f['follower_id'] for f in followers])
        )
...
CREATE TABLE user_relationships (
    ...
    relationship_type VARCHAR(20), -- 'friend', 'family', 'colleague'
    closeness_score FLOAT DEFAULT 0.5,
...
class NotificationService:
    ...
    async def send_notification(self, user_id: str, notification: Notification):
        ...
        # 2. Check if user is online via WebSocket
        ...
        if await self.should_send_push(user_id, notification):
        ...
        # 4. Send email (for important notifications)
